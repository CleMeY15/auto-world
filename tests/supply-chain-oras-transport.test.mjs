...
// this order. Content-addressed blobs are copied without reserialization.
const copiedFixture = () => {
  const fixture = oci.createBootstrapFixture();
  const parent = JSON.parse(fixture.files.get("index.json")).manifests[0];
  const index = Buffer.from(`{"schemaVersion":2,"mediaType":"application/vnd.oci.image.index.v1+json","manifests":[{"mediaType":"application/vnd.oci.image.index.v1+json","digest":"${parent.digest}","size":${parent.size},"annotations":{"org.opencontainers.image.ref.name":"bootstrap"}}]}`);
  assert.equal(index.length, 301);
  assert.equal(sha256(index), "d66f6dd5e3986378305f240943c37e8462f960a50e7c389a2536bad765181776");
...
test("ORAS copied profile rejects every alternate transport serialization or descriptor", () => {
  ...
  const variants = [fixture.files.get("index.json"), canonicalJsonBuffer(parsed),
    ...
    Buffer.from(JSON.stringify({ ...parsed, manifests: [{ ...descriptor, extra: true }] })),
    Buffer.from(JSON.stringify({ ...parsed, manifests: [descriptor, descriptor] })),
    Buffer.from(JSON.stringify({ ...parsed, manifests: [{ ...descriptor, size: descriptor.size + 1 }] })),
