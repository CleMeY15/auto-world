...
function vulnerabilityReport() {
  return {
    ...
    Metadata: { ImageID: subject.imageId, RepoTags: [subject.tag],
      ImageConfig: { os: "linux", architecture: "amd64" }, OS: { EOSL: false } },
    Results: [
      { Target: "debian 13", Class: "os-pkgs", Type: "debian", Packages: [
        { Name: "base-files", Version: "13.8", Release: "1", Epoch: 1 },
      ], Vulnerabilities: [] },
...
function cyclonedxReport() {
  ...
  return {
    ...
    components: [
      ...
      { type: "library", name: "base-files", version: "1:13.8-1", properties: properties("debian") },
...
test("a clean local archive audit returns its local identities without inventing a manifest digest", () => {
  ...
  assert.deepEqual(result.inventory, { resultCount: 2, packageCount: 3, sbomComponentCount: 4 });
...
test("only a fresh report from the pinned scanner for linux amd64 passes", () => {
  for (const mutate of [
    ...
    (report) => { delete report.Metadata.OS.EOSL; },
...
test("CycloneDX must be a bounded one-to-one inventory of all JSON packages", () => {
  for (const mutate of [
    ...
    (sbom) => { sbom.components[1].properties[0].value = "alpine"; },
    ...
    (sbom) => { sbom.components.push({ type: "operating-system", name: "debian" }); },
  ]) {
...
test("all HIGH and CRITICAL findings block because the local policy has no disposition input", () => {
  ...
  for (const finding of [
    { Severity: "HIGH", VulnerabilityID: "CVE-2099-0001", PkgName: "base-files", InstalledVersion: "1:13.8-1", FixedVersion: "" },
    { Severity: "HIGH", VulnerabilityID: "CVE-2099-0001", PkgName: "base-files", InstalledVersion: "1:13.8-1", FixedVersion: "2:13.8-1" },
    { Severity: "CRITICAL", VulnerabilityID: "CVE-2099-0001", PkgName: "base-files", InstalledVersion: "1:13.8-1", FixedVersion: "" },
  ]) {
