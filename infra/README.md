...
`up` verifies ownership, pulls immutable pins, starts the four services, applies the migration and checks usable authenticated APIs. Endpoints bind127.0.0.1 only: PostgreSQL5432, OpenSearch9200, Redis6379, S3-compatible9000. Explicit identity configuration disables SeaweedFS anonymous/default open-key access. Its startup shim copies the read-only host configuration into a private tmpfs owned by upstream UID/GID1000 before the upstream entrypoint drops privileges; it never loosens host credential permissions. This ephemeral configuration is outside data volumes/backups. OpenSearch security is disabled only for this isolated local/CI stack; other local users may reach loopback, so this is not a multi-tenant security boundary.
...
Raw object keys use validated source/run/snapshot IDs. Conditional writes, SHA256 and length readback precede SQL references. Replays compare existing bytes; conflicts never overwrite. Unknown effects produce no SQL metadata. Temporary host upload/download copies are removed in guarded cleanup on success, failure and cooperative cancellation; unexpected files cause a reported cleanup failure, not deletion of unrecognized data. There is no PostgreSQL+S3 distributed transaction: orphan raw objects remain possible. The protocol is not native WORM/ObjectLock and does not stop a privileged user from bypassing it. Policy snapshots remain distinct from operational authorization.
...
Restore-check validates the manifest/archives/platform before extraction, creates a fresh isolated stopped project with empty owned volumes, imports and verifies SQL/raw references, then removes only that temporary project, including its generated credentials after resource removal is verified. Matching PostgreSQL credentials come separately from the original ignored state because the volume includes role password hashes. Never upload archives or credentials as CI evidence. Restore never replaces the active developer database. Original services resume only if they were running before backup; originally stopped services remain stopped.
