...
- Current frontier: TASK-0005 IN_PROGRESS on `codex/task-0005-local-data-infra`, draft PR #7. ADR-0004 preceded code after Architect/Critic plan approval. At `8d4ccd9`, CI `34037895575` passed quality, usable services, migrations, SQL/provenance/privileges, conditional raw races, restart persistence and four dependency recoveries. Positive isolated restore failed `restore_target_nonempty`; the exact-image audit also failed, including candidate-only Alpine scans. Resume the restore correction first; no gate waiver, merge or TASK-0006. See `docs/validation/TASK-0005.md` for evidence and remaining independent final reviews. This host lacks Docker, so Linux CI is the real runtime lane. An owned patched-image supply chain is not silently authorized by a pin adjustment; it requires an explicit decision and reviewed ADR.
