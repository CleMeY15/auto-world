...
- Current frontier: TASK-0005 IN_PROGRESS on `codex/task-0005-local-data-infra`. Independent Architect then Critic approved the infrastructure/recovery plan; ADR-0004 was committed at `f1ccf03` before code. Exact image/action pins were re-resolved from official registries; executable infrastructure, real CI lifecycle/restore/audit and independent implementation reviews remain to be completed. This host lacks Docker, so real Linux CI containers are the required runtime evidence lane.
