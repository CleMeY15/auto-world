...
PR19 retired the publisher at protected merge `1ae9a6693c572e7c8824bcfe20dc1c5a68e4198e` with passing main quality. The subsequent read-only run `34867275879` still failed on a successful same-daemon anonymous `docker pull`, while direct anonymous registry requests returned 401/UNAUTHORIZED and Settings remained Private. The local pull result is not sufficient to prove a remote fetch. A corrected remote manifest control and fresh native evidence are required; neither failed run is relabelled successful.
