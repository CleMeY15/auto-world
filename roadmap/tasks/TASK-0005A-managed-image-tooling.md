...
On23September, PR28 native run35847569210 passed both pristine/corrected EC pairs and produced matching corrected binaries in both jobs. Job2 passed the complete normal suite; the overall run still failed on a work-tree measurement in job1 and the12GiB combined limit during full-tag tests in job2. PR31 addresses cache lifecycle and bounded measurement diagnostics; no source acceptance is claimed. PR30's real scanner run35850661220 proves that the Java database is too old for the unchanged48-hour gate. Its upstream publication workflow is disabled for inactivity. Image auditing/admission remains blocked until a fresh database is available; TASK-0005 and TASK-0006 do not advance.
