...
State: RUN 9 FAILED; backup/restore NOT_VERIFIED. Run 10 is a planned diagnostic retry with bounded failure telemetry, not an accepted proof or image admission.
...
The run-9 public error would mask an earlier candidate/runtime failure if both it and image cleanup failed. Whether such an earlier failure occurred, and its exact native cause, is unknown from this receipt. The focused retry adds bounded primary and cleanup failure context, retains ownership checks, and does not force removal of any ambiguous resource. Run 9 remains FAILED regardless of the retry.

## Required run-10 evidence
...
Rollback removes the run-10 profile and returns the guarded workflow to the reviewed run-9 definition. Preserve immutable run-8/run-9 and any run-10 logs; do not delete any resource whose ownership cannot be proven.
