...
At this inspection checkpoint, the [source diagnostic](TASK-0005A-SEAWEED-SOURCE.md) still required its complete independent build/test/comparison result; that source proof later succeeded in [native run 35884717093](https://github.com/CleMeY15/auto-world/actions/runs/35884717093). The [scanner evidence](TASK-0005A-SCANNER.md) separately records rejection of the stale Java database on 23 September; those failed runs remain failed and supply no exact candidate audit. Under the 2026-09-24 policy amendment, Java age is recorded without an age rejection, while vulnerability database freshness remains required. Source success alone cannot resolve Rust inventory or substitute for an exact-subject audit.
