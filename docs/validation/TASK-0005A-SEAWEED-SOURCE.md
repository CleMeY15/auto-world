...
Run normal Seaweed tests, the [upstream seven-tag suite](https://github.com/seaweedfs/seaweedfs/blob/c5073360007d28385a33426a42ac3e4ec504c5a3/.github/workflows/go.yml), vet and corrected gRPC transport assertions. Redis tests use `RUN_REDIS_TESTS=1` against an exact locked helper, synthetic data and a loopback binding; that helper's diagnostic execution does not admit it. The test compiler/CGO settings are recorded separately from the CGO-free production binary. Linux/386 and cross-OS upstream checks are explicitly `NOT_RUN` in this Linux/amd64 slice.
