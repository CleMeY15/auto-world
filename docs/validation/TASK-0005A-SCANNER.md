...
The download-only container now has a 1536 MiB tmpfs and 2 GiB memory limit. Raw OCI manifests are checked before downloads, with total layer size below 1 GiB and at least 512 MiB temporary headroom. The projected job budget includes the download memory peak and remains capped at 8 GiB; evidence artifacts remain capped at 6 GiB. Twenty-nine targeted tests and lint pass, including the observed Java archive size and oversized-manifest rejection. Actual Linux revalidation is still required.
