...
Status: Diagnostic PASS on 2026-09-14; producer retired in this change, delivery gates recorded in the associated PR. No image architecture accepted.
Date: 2026-09-08
...
Both calls retain repo `CleMeY15/auto-world`, GitHub OIDC issuer, source ref, source SHA, signer SHA, denial of self-hosted runners, SLSA provenance/v1 and JSON output. Both must succeed and produce one identical attestation and verified content. The real CLI output requires excluding only `verificationResult.verifiedIdentity` from deep equality: this field echoes the differing exact-SAN/workflow-regexp input policies. Every other field, including the certificate, statement and signed object, remains compared. Multiple JSONL bundles, ambiguous result sets, differing verified content or file/bundle substitution fail closed. The wrapper delegates signatures, certificates, transparency and policy verification to official gh; it implements no cryptography.
...
The three required security negatives are the valid branch bundle under main policy, changed fixture bytes against the genuine main bundle, and a wrong expected workflow against that same genuine main bundle. Missing/truncated bundles, unavailable CLI, timeouts and network errors remain operational/input errors. Actual non-TTY gh2.98.0 omits progress phases and returns only its final Sigstore/source diagnostic. Classification accepts these exact bounded messages and also requires the unchanged bundle's successful positive control and the prescribed byte/policy perturbation; no arbitrary nonzero exit proves a mismatch. The branch's signer-workflow call additionally requires the explicit certificate source mismatch reported by gh. Numeric exits are retained separately from classification codes.
...
## Observed outcome, 2026-09-14

Runs 34804951573 (M, commit 66af143) and 34805000357 (B, commit 6042a5d), both attempt 1, succeeded. Both genuine bundles passed their own exact identities; all three controlled negatives were proved after one bounded local output-format correction. No new issuance followed. Workflow 353251429 was disabled after capture and its exact bytes archived outside `.github/workflows/`. The [evidence report](../validation/TASK-0005-CANARY.md) retains initial failure, corrected real process evidence, hashes, run metadata and retirement gates. Final code/security and architecture review of the evidence/removal PR is required before its normal protected merge.
