...
GHCR's documented first-publication default is private, but actual privacy must be proven before candidate layers are written. The separate activation plan must establish a non-sensitive first-write/package-creation sequence, actual visibility/access checks and anonymous pull denial. Use only the existing namespace and deterministic package names. Authorized read-only Actions pull must succeed; anonymous retrieval must fail. No local token expansion is assumed: current local authentication lacks `read:packages`.

GitHub also warns that granting a public repository access to a private package may expose it to forks. Private visibility alone is insufficient. Before candidate layers or private evidence are written, prove that inherited permissions, repository linkage and Actions grants do not permit untrusted public-repository readers, forks or `pull_request` contexts to read the package. Inventory every workflow/job with package-read access and forbid running PR/fork-controlled code in any such job, including indirect checkout, artifacts and reusable workflow inputs. Require actual denied-read tests from authenticated fork/PR contexts as well as the anonymous control, paired with a valid authorized read. Those probes use only the harmless initial content.

The concrete Architect then Critic gate must examine these permission boundaries before the harmless first write and require their actual proofs before candidate writes or admission. A public-repository `GITHUB_TOKEN` is not presumed capable of satisfying this boundary. If the existing topology cannot establish it, private publication remains blocked; complete scanner work independently and resolve the required identity/topology authority before proceeding. Do not silently add a publisher repository, credential, service or bypass to satisfy the gate.
