...
Private unadmitted staging may precede final image audits under the separately approved ADR7 plan. The vulnerability database freshness and both databases' valid, ordered, non-future timestamps and byte identities are required at actual scans, signing/admission and supported use; they do not prohibit ordinary recipe design or separately authorized unadmitted staging. The Java database age is recorded without a maximum-age rejection under the 2026-09-24 user decision. Older Java data limits current Java-vulnerability coverage and must never be described as fresh or have its timestamps changed.
