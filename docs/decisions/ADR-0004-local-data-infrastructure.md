...
2026-09-07 scoped restore correction: CI34142987078 identified `restore_target_nonempty_object-store` before extraction. The official SeaweedFS4.45 [Dockerfile](https://github.com/seaweedfs/seaweedfs/blob/4.45/docker/Dockerfile.local) creates `/data/filerldb2` in the image, and Docker [copies image contents into empty volumes by default](https://docs.docker.com/engine/storage/volumes/). Disable copy-up only on the `object-store-data` mount with `volume.nocopy:true`; keep all four empty-target checks and do not erase or exempt skeleton directories. The existing upstream [entrypoint](https://github.com/seaweedfs/seaweedfs/blob/4.45/docker/entrypoint.sh) repairs `/data` ownership before dropping to UID/GID1000. No new permissions shim or global `nocopy` is introduced. Real CI must prove fresh initialization and restored raw data, plus UID/GID1000 for `/data`, filer metadata and the running service; static configuration alone does not validate this correction.
