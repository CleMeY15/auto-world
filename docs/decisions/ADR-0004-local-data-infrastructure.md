...
`infra/compose.json` is a strict JSON/YAML-subset document, validated with Node built-ins and real `docker compose config`. Four service keys: `postgres`, `opensearch`, `redis`, `object-store`. No globally fixed container_name, host network, privileged mode, Docker socket or broad host mount. Use an internal project bridge and separately owned named volumes. All published ports bind127.0.0.1, defaults5432/9200/6379/9000 respectively; integration uses ephemeral loopback ports and discovers actual mappings.
...
SeaweedFS uses `server -dir=/data -s3` without a conflicting filer.toml: volume, master and implicit `/data/filerldb2` metadata share the persistent volume. Only the S3 endpoint is published. Its default open-key behavior is prohibited: mount a generated explicit identity configuration, no anonymous identity, only the required Read/List/Write bucket scope. Unsigned/wrong-key denial, bucket/object persistence and conditional writes must pass on the exact image, including after restart.
...
Use source/listing/version/observation/raw prefixes and bounded fields, lowercase SHA256, finite supported timestamptz(3), confidence0..10000 and known field/method/status discriminants. Keep measurement/currency value JSONB after public validation, not a lossy SQL reinterpretation. Policy metadata is an already validated V1 snapshot, not an operational authorization flag. Do not duplicate the entire TypeScript parser in SQL or add speculative JSONB indexes.
