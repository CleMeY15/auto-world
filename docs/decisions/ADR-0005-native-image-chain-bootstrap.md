...
Inspection of the hash-verified official Go 1.26.8 Linux archive found exactly two PAX path records for Unicode test filenames. The pinned Trivy source contains exactly three relative test-fixture symlinks. Native compiler/source validation therefore uses separate, closed profiles for these exact records and links, with their paths, payloads/targets and identities fixed before extraction. Unknown extensions, links, traversal, duplicate entries and all existing resource-limit violations still fail. The OCI entrypoint retains its unconditional PAX/link refusal; these native profiles do not apply to OCI or service layers.
...
Before any candidate execution, retain the six verified build records as immutable in-memory expectations and stage all consumed native outputs against their hashes. Never reread a candidate-writable record to establish a new expected subject after a scanner or CLI has run. Native readers reject symbolic and hard links, and verify subject/scanner/fixture/database bytes after execution. Audit receipts name the explicitly selected GHCR DB repositories; default mirror selection is not recorded as a proven GHCR origin. Reserve the complete audit's input/copy/report budget before duplicating DB and subject data.

The audit budget follows two phases under unchanged 6 GiB artifact and 8 GiB job limits. Let `B` be the sum of the four staged binaries, `M` the verified candidate-matrix bytes, `F` the bounded fixture reserve, and `D` the four database/metadata sizes. The artifact reserve is `A = B + 880 MiB + F`; the scan peak is `M + A + max(binarySizes) + F + 2D`; the packaging peak is `M + A + 2 GiB + D`. The scan formula requires each subject workspace to contain exactly its verified binary and to be removed before the next subject. Cache must contain exactly the four verified database files and HOME must remain empty. Both are removed after final identity and audit validation, before packaging. The fixture helper receives a precreated scratch directory owned by the same lifecycle; its five fixed environment directories must be empty after each child command. The two `F` reserves cover retained fixture materials and transient fixture scratch separately: for the pinned corpus, these are 3,201,696 and at most 1,349,660 bytes respectively, each below 4 MiB. If owned cleanup cannot be proved, packaging is refused. The unchanged packager copies, verifies and releases one known source file at a time; its maximum transient copy is 2 GiB. These calculations bound the retained artifact lifecycle, not arbitrary operating-system writes by a candidate. A budget refusal records only fixed numeric sizes, totals and a closed reason.

### Managed Docker diagnostic preparation

The failed PR7 scanner comparison requires an explicit additional managed-service trust boundary, separate from the ten native-build utilities: the installed Docker CLI/daemon/containerd/OCI runtime, plus read-only package metadata tools `dpkg-query` and `apt-cache`. A bounded independent review approved an inventory-first preparation step. This step records executable hashes, installed package/version/origin, Engine/runtime metadata, runner/workflow identities and pre/post local image-store inventory. It acquires only `aquasec/trivy@sha256:ee940acbf1f58ebadb42d01434ce4609530bf1b52536afbd1eee66cd7123c5c9` as public data through an empty Docker client configuration and a fixed local Unix socket. Independently fetched, hash-verified index/manifest bytes must establish the pinned PR7 parent/child/config relation; descriptor size and available-disk checks precede the pull. No container is run, created or built during inventory, and no existing image is deleted.

The inventory is a `diagnostic_tcb_proposal`, not native-tool or image admission. Actual baseline comparison remains explicitly refused until its real receipt receives independent review and the separate comparison implementation is verified. That later lane must use the identical captured DB and fixture bytes, `--pull=never`, no network, a read-only root and input mounts, no capabilities or socket mount, and bounded raw JSON reports. The baseline remains failed/non-admitted; it cannot waive candidate findings. Docker is not an undeclared builder or a production/publisher capability, and this preparation does not unblock TASK-0005B.

The baseline diagnostic script is separately scoped and identified by its own recipe hash in the diagnostic receipt. The native build/audit recipes close their complete imported module set. Changes to this diagnostic script do not silently claim a change to rebuilt native source or supply an admission approval.
...
### Reviewed admission proposal

Before a dormant merge, a separate local data-only postprocessor must derive and validate `infra/supply-chain/native-admission.json`. Its only state is `reviewed_admission_proposal`, its intended stage is `merged_pending_activation`, and its activation remains blocked with an empty capability array. No workflow automatically writes or promotes this proposal. The producer calls no candidate, subprocess or network API. It may reuse existing pure validators, including their inert transitive imports, provided the complete admission import closure is hashed and independently reviewed and no execution path is callable through its entry points.

The proposal binds the six actual build records, four byte-identical repeated outputs, source/material/native recipe closure, reproducibility receipt, ORAS/Cosign CLI proofs, all four native audits and their exact scanner/DB/SBOM/report bytes, fixture controls, and the matching baseline diagnostic. The baseline stays `failed_non_admitted`. Closed inventories, existing size budgets, repeated file-identity checks and exclusive atomic output apply; a partial or substituted bundle cannot produce a proposal.

Semantic validation must consume the same bounded-read bytes whose identities are bound into the proposal. Captured audit identities are passed into the shared file verifier and compared before parsing; the summary, package receipt, fixture manifest and TCB inputs retain their original parsed-byte identities. Rehashing after validation alone does not prevent a file from being replaced and restored between reads. The baseline receipt must name the exact image child digest from its freshly verified TCB.

A canonical repository context is prepared and verified outside the postprocessor. It binds the complete CI identity, PR head/base/synthetic merge SHA and tree, exact workflow and recipe bytes, and external run start/end times. A PR's synthetic merge need not be an ancestor of a later branch HEAD; no local ancestry, checkout cleanliness or GitHub authenticity claim is inferred from context JSON. Each audit timestamp must lie within the recorded run, with `UpdatedAt <= DownloadedAt <= CreatedAt` and at most 48 hours from database update to scan. This validates historical evidence without presenting it as a new scan.

Derive the canonical payload before independent review. Exactly two distinct roles, `code_security` and `architecture`, must approve the same payload and implementation/material/run identities; their identities must differ from the authors/executors. Each receipt binds the report path/hash/size and either the prior immutable report commit or an external review event. It never refers to the commit containing that same receipt. Specialist reports may be linked as supporting evidence. Finalization re-derives the payload and binds both receipts; hash validation records the assertions, while actual reviewer independence remains an externally verified review gate. The proposal grants no installation, custody, publishing or execution authority.
...
The generated source layout and the pinned ORAS copy have distinct closed transport-index profiles. The source remains lexicographically canonical without root `mediaType`. ORAS 1.3.4's locked oras-go/v2.6.2 marshals its destination index with root `mediaType` and Go struct field order. Its store indexes both pushed manifests, then tags the parent: the destination contains the `bootstrap` parent followed by the digest-only linux/amd64 child. This exact profile is `4d87d7560b88f51a04b219279483621ec4fd32744e67446a5c12a9f97ae0bba2`, 503 bytes, matching the identity observed in native run 34200538047. The preceding 301-byte prediction omitted the child and is rejected. The copied-layout validator constructs its own trusted expected fixture and requires exactly five paths: that exact index, unchanged `oci-layout`, and three unchanged content-addressed blobs. It neither normalizes observed bytes nor accepts caller-supplied expectations. Any serialization or content drift fails. A fresh native CLI run must pass the repaired validator before this supports admission; the original source validator remains unchanged.

The copy diagnostic publishes the five actual file identities only. Expected identities are derived from the internally constructed, reviewed fixture and the pinned transport-profile regression above; they are deliberately compared outside the log rather than duplicated as additional diagnostic fields. The runtime validator still requires exact byte equality for every expected file.
