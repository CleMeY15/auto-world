...
The generated source layout and the pinned ORAS copy have distinct closed transport-index profiles. The source remains lexicographically canonical without root `mediaType`. ORAS 1.3.4's locked oras-go/v2.6.2 marshals its destination index with root `mediaType` and Go struct field order. Its predicted index is exactly `d66f6dd5e3986378305f240943c37e8462f960a50e7c389a2536bad765181776`, 301 bytes. The copied-layout validator constructs its own trusted expected fixture and requires exactly five paths: that exact index, unchanged `oci-layout`, and three unchanged content-addressed blobs. It neither normalizes observed bytes nor accepts caller-supplied expectations. Any serialization or content drift fails. Public identity-only diagnostics must confirm these predicted destination bytes on Linux before accepting the CLI proof; the original source validator remains unchanged.
